        ...
        implements Tool
{
    ...
    public static class DomainMapper
            extends Mapper<LongWritable, WARCRecord, Text, LongWritable>
    {
        private static final LongWritable ONE = new LongWritable(1L);

        @Override
        protected void map(LongWritable key, WARCRecord value, Context context)
                throws IOException, InterruptedException
        {
            try {
                // extract the top domain
                URI uri = new URI(value.getHeader().getTargetURI());
                String domain = InternetDomainName.from(uri.getHost()).topPrivateDomain()
                        .toString();
